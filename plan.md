...
In 0.39.0: #185, a follow-up to #178. The forwarder's registry check ran once, early - before the settle-gate wait and the AGW/RHP connect round trip, both of which take real time - so a live capture still showed the glare pattern: an inbound session registered in that gap, after the check passed but before the outbound SABM reached BPQ. `PeerSessionRegistry.TryAcquire` closes it: `BearerSwitchingOutboundTransport` now checks-and-acquires atomically, under the registry's own lock, immediately before the dial rather than relying on the forwarder's earlier snapshot. A busy peer found there throws `PeerSessionBusyException`, which `Dappsv1SessionBackhaul` turns into the same `Deferred` result as any other #178 non-send. The forwarder's early check is kept as a fast path (skips the settle-gate wait and connect attempt outright in the common case); it is no longer the only guard.

In 0.40.0: #188 and #189, from Kevin M0AHN's session-traffic analysis (#187), which showed 11 WPS replication messages taking 77 s over 6 AX.25 connections. #188: the forwarder used to open a fresh session for every queued message; each run now groups its queue by next hop and hands each group to the bearer as an `IBackhaulBatch`, and `Dappsv1SessionBackhaul` carries it on one session - pushes back to back, one `routes` pull, `rev`, then a re-check that pushes anything queued meanwhile for that neighbour (up to 20 per batch) and revs again, so `rev` is always the last exchange. Residual TTL is worked out when a message is handed out, not when the run started. Once a session has carried an exchange, the link ending defers the next message (no cooldown) instead of failing it. The `data` line and payload go out in one write, so one I-frame. #189: `fmt=z1`, zstd with a shared raw-content dictionary (`dapps.client/Compression/payload-v1.dict`, SHA-256 pinned by a test, `*.dict binary` in `.gitattributes`), used when it saves at least 32 bytes: a 197-byte WPS post goes as 67. A node that doesn't hold the offered version, or can't decode what arrived, refuses and gets the message plain on the same session, and the rest of that session goes plain. `CompressionEnabled` system-wide (default on) and per neighbour. No negotiation, as DAPPS isn't deployed anywhere yet. Not yet: the connection tail (#187 proposal 9), pipelined offers (4), and waking the forwarder when a message is submitted rather than on its 5 s tick.

In 0.41.0: #190 and #191, the latency half of #187. #190: the forwarder ran on a fixed 5 s tick, so every message waited 2.5 s on average at every hop before anything happened. It now runs when `ForwarderWakeup` is poked - `Database.SaveMessage` on every queued message (after a 250 ms settle so a burst goes as one batch), `PeerSessionRegistry` when the last session with a peer ends - when the next destination comes out of reconnect cooldown, and every 30 s as a fallback for changes nothing signals. #191, the connection tail (#187 proposal 9): after a session that pushed and ended cleanly the caller sends `tail <s>`, the callee answers with the lower of both ends' `SessionTailSeconds` (default 120, 0 = off, at most 600, per neighbour), and the link stays up until it has been quiet that long, then `quit`. While held, the forwarder hands new batches for the peer to the held session (`IDappsBackhaul.TryHandToOpenSession`) or, when the peer has a session open with us, to that `InboundConnectionHandler` via `InboundSessionDirectory`, which writes `pending` while idle so the peer revs; flood copies go the same way. The caller skips `pending` wherever it reads a reply. A held session reads through `PumpedReadStream` so it can wait on the peer, new work or its timer without cancelling a read mid-line, and runs alongside the forwarder: messages are claimed at hand-out (and checked still queued) and outcomes recorded one at a time. A held link closes after 30 minutes however busy; a glare-served session is never held. Not yet: pipelined offers (#187 proposal 4).
