...
DAPPS keeps track of which peers it has a session open with, in either direction, and never dials a peer that has one. If we're holding a link open to the peer, its traffic goes on that link. If the peer has a session open with us, the traffic is handed to that session, which tells the peer it has mail (`pending`) and sends it when the peer asks with `rev`; the log line is `Handed <n> message(s) for <peer> to the session it has open with us`. Otherwise the traffic stays queued (`Deferring <n> message(s) for <peer>: an inbound session with it is already open`) and goes as soon as that session ends. The guard is one-sided (each node only stops itself dialling into a live session), so if you still see the pattern above, check that the *other* node is also on a DAPPS version that has it.
